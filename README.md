...
- **An embedding backend** configured in ST's Vector Storage — Memory Loom uses ST's native vector API. Ollama with an embedding model (e.g. `qwen3-embedding`) works well, as do the other sources ST supports (OpenAI, Cohere, etc.).
...
2. **Embedding** — make sure ST's Vector Storage is pointed at a working embedding model. Memory Loom rides on that configuration.
...
The embedding model is configured in ST's Vector Storage, not in Memory Loom directly — use whatever you prefer. Remember to tune the similarity threshold to match it (the built-in default leans strict for some embedding models).
...
*Memory Loom is a community SillyTavern extension. It orchestrates your own configured models and ST's native vector storage; it ships no models of its own.*
